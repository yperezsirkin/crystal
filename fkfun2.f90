...
if(denpart(ix,iy,iz).lt.0.5) then
!if(denpart(ix,iy,iz).gt.0.5) then
!fprot = expmuprot/(vprot*vsol)
!xprot = expmuprot
!else
  do jx = -protR, protR
  do jy = -protR, protR
...
!!!!!!!!!!
     ...
     proprot = proprot *(xh(ax, ay, az)**(protvol(jx, jy, jz)/vsol))

  enddo
  enddo
  enddo

  fprot(ix,iy,iz) = proprot/(vprot*vsol) 
...
!!!!!!!!!
    ...
    xprot(ax, ay, az) = xprot(ax, ay, az) + protvol(jx, jy, jz)*proprot/(vprot*vsol)

  enddo !jz
  enddo !jy
  enddo !jx

endif !espacio donde calculo 
enddo !iz
enddo !iy
enddo !iz
...
!!!!!!!!
  ...
  !  xprot(ix,iy,iz) = xprot(ix,iy,iz)*dexp(xtotal(ax,ay,az,ip)*protpol(jx,jy,jz)*eprotpol) 
...
!!!!PBC!!!!
 ...
 ! az = PBCSYMI(az,dimz)
  ...
  !xprot(ax,ay,az) = xprot(ax,ay,az) + fprot(ix,iy,iz)*protvol(jx,jy,jz) 
...
!  az = PBCSYMI(az,dimz)
 ...
 !   xpot(ax, ay, az, im) = xpot(ax,ay,az, im)*dexp(protpol(jx,jy,jz)*xprot(ix,iy,iz)/(vprot*vsol)*eprotpol)
