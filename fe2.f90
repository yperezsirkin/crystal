...
real*8 sumprot,sumprot1, sumprot2 !yamila borrar
! MPI
...
!termino entropico traslacional y muN with respect to the bulk
      ...
      if(fprot(ix,iy,iz).ge.1E-15) then  
     ! if(denpart(ix,iy,iz).lt.05) then

      fv = (1.0-volprot(ix,iy,iz))    
      fv2 = (1.0 -denpart(ix,iy,iz))
      if(fv2.le.0.5) fv2 = 0.0
      F_prot = F_prot + fprot(ix,iy,iz)&
      *(dlog(fprot(ix, iy, iz)*vsol) - 1.0 - dlog(expmuprot) + dlog(vprot))*fv2 

      F_prot = F_prot - F_protbulk*fv2
...
!!!!PBC!!!!
 ...
 ! az = PBCSYMI(az,dimz)
  ...
  !fv2=(1.0-volprot(ax,ay,az))
   ...
   !do im = 1, N_monomer
 !  F_protpol = F_protpol - avpol(ax,ay,az,im)*xprot(ix,iy,iz)*protpol(jx,jy,jz)*eprotpol*fv*fv2
  ! enddo
 ! enddo
 ! enddo
 ! enddo
...
! enddo
 ...
 ! F_protpol = F_protpol/vpol/vsol/vprot/vsol*delta**3
...
! minimal F
      ...
      enddo
        ...
        sumel=0.0
        sumdiel = 0.0
        sumprot =0.0 !yamila borrar
        sumprot1 = 0.d0       !yamila borrar
        sumprot2 = 0.d0 !yamila borrar
      ...
      fv=(1.0-volprot(ix,iy,iz))
      fv2= (1.0 -denpart(ix,iy,iz)) !yamila
      if(fv2.le.0.5) fv2 = 0.0  !yamila

           sumpi = sumpi+dlog(xh(ix, iy, iz))*fv     !termino osmotico
           sumpi = sumpi-dlog(xsolbulk)*fv
     
     sumrho = sumrho + ( - xh(ix, iy, iz) -xHplus(ix, iy, iz) &
    - xOHmin(ix, iy, iz)-(xpos(ix, iy, iz)+xneg(ix, iy, iz))/vsalt)*fv-fprot(ix,iy,iz)*vsol*fv2 !xprot(ix,iy,iz)/vprot)*fv!   yamila


           sumrho = sumrho - ( - xsolbulk -xHplusbulk &
       -xOHminbulk - (xposbulk+xnegbulk)/vsalt)*fv+xprotbulk/vprot*fv2 ! sum over  rho_i i=+,-,s  yamila

         sumel = sumel - qtot(ix, iy, iz)*psi(ix, iy, iz)/2.0 
      
         sumel = sumel + volq(ix,iy,iz)*psi(ix,iy,iz)*vsol                   

      !   sumprot = sumprot -xpos(ix, iy, iz)/vsalt*fv +xposbulk/vsalt*fv        !borrar
         sumprot = sumprot + (-fprot(ix,iy,iz) + xprotbulk/vprot/vsol)*fv2*delta**3  !yamila borrar         
         sumprot1 = sumprot1 -fprot(ix,iy,iz) *fv2*delta**3 !yamila borrar
         sumprot2 = sumprot2 +xprotbulk/vprot/vsol*fv2*delta**3  !yamila borrar
      ...
      write(stdout,*) 'sum prot', sumprot,sumprot1, sumprot2
      endif
